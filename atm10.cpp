...
int main()
{
	...
	{
	printf("%s",name);
		PlaySound("atmswelcom.wav",NULL,SND_SYNC);
	}
	else
	{
    PlaySound("atmsound1.wav",NULL,SND_SYNC);
	scanf("%s",name);	
    }
    if(c==0||b==2)
    readfile();
    ...
    printstar('*',60);
    printf("\n\nenter your pin=");
    PlaySound("atmspin.wav",NULL,SND_SYNC);
	count=ayyappa();
	if(count!=0)
	{
		for(r=2;r>0;r--)
		{
		    ...
		    PlaySound("atmsinc.wav",NULL,SND_SYNC);
		    printf("\nenter your pin=");
		    PlaySound("atmspin.wav",NULL,SND_SYNC);
		    count=ayyappa();
	    }
	...
	if(count!=0)
	{
		...
		PlaySound("atmsinctry.wav",NULL,SND_SYNC);
		exit(0);
	}
	...
	{
	printf("\nenter your choice:");
		PlaySound("atmschoice.wav",NULL,SND_SYNC);
	scanf("%d",&a);
	system("cls");
	system("color 7A");
    }
	...
	case 1:
		{
			...
			PlaySound("atmscredit.wav",NULL,SND_SYNC);
			scanf("%d",&y);
			total=total+y;
			system("cls");
			PlaySound("atmstransac.wav",NULL,SND_SYNC);
			sleep(3);
	...
	case 2:
	{
		...
		PlaySound("atmsdebit.wav",NULL,SND_SYNC);
		scanf("%d",&y);
		if(total>=y)
		{
	    total=total-y;
	    ...
	    y=0;
	    PlaySound("atmsinsu.wav",NULL,SND_SYNC);
	    }
	    statement();
	    break;
		}	
	...
	if(count!=0)
	{
		...
		{
		printf("\nenter your new pin:");
		PlaySound("atmsnpin.wav",NULL,SND_SYNC);
		for(i=0;i<4;i++)
	{
		ch=getch();
		pass1[i]=ch;
		...
		PlaySound("atmssnp.wav",NULL,SND_SYNC);
        }
        else
    ...
    printf("\n");
    printstar('=',120);
    PlaySound("atmsstate.wav",NULL,SND_SYNC);
	break;	
    }
    writefile();
    system("color 7C");
        ...
        printf("\n*thank you*\n*visit again*");
        PlaySound("atmssuccess.wav",NULL,SND_SYNC);
        }
        else
        {
        printf("\n*thank you*\n*visit again*");
        PlaySound("atmsthank.wav",NULL,SND_SYNC);
    }
    PlaySound("atms1p.wav",NULL,SND_SYNC);
    o=getch();
    s++;
    system("cls");
    if(o=='1')
	...
	void readfile()
	{
		...
		PlaySound("atmsdont.wav",NULL,SND_SYNC);
		scanf("%d",&i);
		if(i==1)
		{
			...
			PlaySound("atmscreate.wav",NULL,SND_SYNC);
			printf("\npress 1 to continue");
			PlaySound("atms1p.wav",NULL,SND_SYNC);
			getch();
			system("cls");
